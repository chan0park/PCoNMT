...
if True:
    ...
    for line in addone(codecs.open(opt.src, "r", "utf-8")):
        ...
        # predBatch, predScore, knntime, attn, out, outScore,
        # if tgtF is not None:
        ...
        for b in range(len(predBatch)):
            ...
            count += 1
            # try:
            #     content = " ".join([y for x in predBatch[b][0] for y in x.split("_")])
            # except:
            #     content = " ".join([encode_or_decode(y) for x in predBatch[b][0] for y in x.split("_")])
            try:
    ...
    if opt.save_att:
        ...
        outF_model.flush()
        # zip_output = subprocess.check_output(["zip -r {}.zip {}".format(opt.save_dir.replace(".pt",""), opt.save_dir )], shell=True)
        # subprocess.check_output(["rm -r {}".format(opt.save_dir)], shell=True)
