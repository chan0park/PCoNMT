...
class GlobalAttentionOriginal(nn.Module):
    r"""
    ...
    * Bahdanau Attention (mlp):
    ...
    def __init__(self, dim, coverage=True, attn_type="mlp",
                 attn_func="softmax"):
        super(GlobalAttentionOriginal, self).__init__()
    ...
    def forward(self, source, memory_bank, memory_lengths=None, coverage=None):
        ...
        if self.mask is not None:
            align.data.masked_fill_(self.mask.view(-1, self.mask.size(-1)), -float('inf'))
        ...
        if one_step:
            ...
            if coverage is not None:
                covered = F.softmax(align.view(batch*target_l, source_l)/self.temp, -1)
