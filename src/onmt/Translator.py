...
class Translator(object):
    def __init__(self, opt):
        ...
        encoder = onmt.Models.Encoder(model_opt, self.src_dict, model_opt.fix_src_emb, use_cov=True)
    ...
    def translateBatch(self, srcBatch, tgtBatch, alignBatch=None):

        batchSize = srcBatch[0].size(1)
        beamSize = self.opt.beam_size
        knntime = 0.0
        #  (1) run the encoder on the src
        encStates, context, cov = self.model.encoder(srcBatch, is_cov=True)
        init_cov = deepcopy(cov)
        init_cov = torch.max(init_cov, dim=-1)[1].float()
        ...
        for i in range(self.opt.max_sent_length):
            ...
            for i_b, b in enumerate(beam):
                if not b.done():
                    c = b.getCurrentState()
                    if self.opt.replace_unk and i != 0:
            ...
            cov_beam = cov.view(remainingSents, beamSize, -1).contiguous()
            init_cov_beam = init_cov.view(remainingSents, beamSize, -1).contiguous()
            ...
            for b in range(batchSize):
                ...
                idx = batchIdx[b]
                if not beam[b].advance(wordLk.data[idx], attn.data[idx], out.data[idx], cov_beam.data[idx], init_cov_beam.data[idx]):
                    active += [b]
        ...
        allHyp, allScores, allAttn, allOut, allOutScores, allCov, allInitCov = [], [], [], [], [], [], []
        ...
        for b in range(batchSize):
            ...
            hyps, attn, out, score, cov, init_cov = zip(*[beam[b].getHyp(times, k) for (times, k) in ks[:n_best]])
        ...
        return allHyp, allScores, allAttn, allOut, allOutScores, allCov, allInitCov, knntime

    def translate(self, srcBatch, goldBatch, alignBatch=None):
        #  (1) convert words to indexes
        dataset = self.buildData(srcBatch, goldBatch)
        src, tgt, _, _, indices = dataset[0]

        #  (2) translate
        pred, predScore, attn, out, outScore, cov, init_cov, knntime = self.translateBatch(src, tgt)
        pred, predScore, attn, out, outScore, cov, init_cov, = list(zip(*sorted(zip(pred, predScore, attn, out, outScore, cov, init_cov, indices), key=lambda x: x[-1])))[:-1]
        ...
        return predBatch, predScore, knntime, attn, out, outScore, cov, init_cov
