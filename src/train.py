...
# cov specific
parser.add_argument('-fert_mode', default='h', type=str, help="[emb|h|emh]")
parser.add_argument('-cov_lr', type=float, default=0.0005,
                    help='Number of training epochs')
parser.add_argument('-fert_dim', type=int, default=2,
                    help='Number of training epochs')
parser.add_argument('-cov_hidden_dim', type=int, default=4096,
                    help='Number of training epochs')
parser.add_argument('-cov_w', type=int, default=3,
                    help='weight for cov loss')
parser.add_argument('-fert_ep', type=int, default=9,
                    help='Number of training epochs')
...
def evalCov(model, loss_cov, cov_data_val):
    ...
    with tqdm(enumerate(range(batch_tot_num)),total=batch_tot_num, leave=False, dynamic_ncols=True) as pbar:
        for i, batch_num in pbar:
            ...
            inputs, _labels_ = zip(*cov_data_val[batch_num*opt.batch_size:(batch_num+1)*opt.batch_size])
            total += len(_labels_)
...
def trainModel(model, trainData, validData, dataset, target_embeddings, target_uni_embeddings, target_ngram_embeddings, optim):
    ...
    def trainCov(epoch, cov_data, is_cov=True, pre_train=False, uni_train=False):
        batch_tot_num = int(len(cov_data)*1.0/opt.batch_size)
        # batch_tot_num_val = int(len(valid)*1.0/batch_size)
        running_loss = 0.0
        total_loss = 0.0
        correct = 0
        total = 0
        with tqdm(enumerate(range(batch_tot_num)),total=batch_tot_num, leave=False, dynamic_ncols=True) as pbar:
            ...
            for i, batch_num in pbar:
                # get the inputs; data is a list of [inputs, labels]
                inputs, _labels = zip(*cov_data[batch_num*opt.batch_size:(batch_num+1)*opt.batch_size])
                inputs= Variable(torch.stack(inputs)).cuda()
    ...
    def getCovData(data, model):
        start = time.time()
        len_data = len(data) if not opt.test else min(len(data), 1000)
        res = []
        thre = 1.0 if opt.fert_dim>2 else 0.0
        with tqdm(enumerate(range(len_data)),total=len_data, leave=False, dynamic_ncols=True) as pbar:
            for i in range(len_data):
                batch = data[i][:-1] # exclude original indices
                src, targets = batch[0], batch[3]
                _, _, _, cov_inp = model.encoder(src, is_cov=False)
                fert_dim = cov_inp.size(2)
                targets = targets.transpose(1,0).contiguous().cpu().data.view(-1)
                cov_inp = cov_inp.cpu().data.view(-1,fert_dim)
                res.extend(zip(cov_inp, targets))
        random.shuffle(res)
        if opt.downsample:
            print("down sampling")
            non_one_samples = [(v,f) for v,f in res if f > thre]
            num_non_one = len(non_one_samples)
            print("total non one samples: "+str(num_non_one))
            one_samples = []
            for v,f in res:
                if f <= thre:
                    one_samples.append((v,f))
                    if len(one_samples) == num_non_one:
                        break
            num_one = len(one_samples)
            print("total one samples: "+str(num_one))
            res = non_one_samples + one_samples
            random.shuffle(res)
        return res

    def trainEpoch(epoch, is_cov=False, pre_train=False, uni_train=False):
        ...
        len_data = len(trainData) if not opt.test else min(len(trainData), 1000)
        for i in range(len_data):
        # for i in range(10):
    ...
    for epoch in range(opt.start_epoch, opt.start_epoch+opt.epochs):
        ...
        if epoch == opt.start_epoch + opt.pre_ep:
            ...
            uni_train=False
            cov_train=True
            cov_data = getCovData(trainData, model)
            cov_data_val = getCovData(validData, model)

        
        if epoch == opt.start_epoch + opt.pre_ep + opt.fert_ep:
        ...
        if not cov_train:
            train_loss, train_acc = trainEpoch(epoch, is_cov=cov_train, pre_train=pre_train)
        else:
            train_loss, train_acc = trainCov(epoch, cov_data, is_cov=cov_train, pre_train=pre_train)
        if uni_train:
            _, _ = trainEpoch(epoch, is_cov=cov_train, pre_train=pre_train, uni_train=uni_train)
        ...
        print('Validation per step accuracy: %g' % (other_loss))
        ...
        # if best_other_loss > other_loss:
            ...
            if pre_train:
                ...
                torch.save(checkpoint,
                    '%s_bestmodel_fert.pt' % opt.save_model)
...
def main():
    ...
    trainData = onmt.Dataset(dataset['train']['src'],
                             ...
                             dataset['train']['align'], opt.batch_size, opt.gpus, fert_dim=opt.fert_dim)
    validData = onmt.Dataset(dataset['valid']['src'],
                             dataset['valid']['tgt'], 
                             dataset['valid']['tgt_uni'], 
                             dataset['valid']['align'], opt.batch_size, opt.gpus,
                             volatile=True, fert_dim=opt.fert_dim)
    ...
    encoder = onmt.Models.Encoder(opt, dicts['src'], opt.fix_src_emb, use_fert=True)
